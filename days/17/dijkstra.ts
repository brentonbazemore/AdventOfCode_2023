// https://github.com/kaisnb/dijkstra-ts/tree/master
...
export const dijkstra = <T>(adapter: IGraphAdapter<T>, startNode: T, finishNode?: T) => {
...
export const findShortestPath = <T>(result: { costs: Record<NodeKey, number>; parents: Record<NodeKey, T> }, adapter: IGraphAdapter<T>, startNode: T, finishNode: T) => {
  const getKey = (node: T) => getNodeKey(node, adapter);
  const { costs, parents } = result;
